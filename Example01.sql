...
alter table Reservation add roomNumber INT;

# - 컬럼 삭제
# ALTER TABLE 테이블명 DROP 필드명;
alter table Reservation DROP roomNumber;
alter table Reservation DROP telNumber;
...
INSERT INTO Reservation (name, reservationDate, roomNumber) VALUES ('김민수', '2023-01-17', 3134);
INSERT INTO Reservation (name, reservationDate) VALUES ('홍길동', '2023-01-17');
...
UPDATE Reservation SET reservationDate = '2023-01-30' WHERE name = '김민수';
...
# Reservation 테이블에서 roomNumber 컬럼에 값이 존재하는 레코의 모든 값을 선택
SELECT * FROM Reservation WHERE roomNumber IS NOT NULL;
...
# reservationDate가 2023-01-01 부터 2023-02-28 까지인 레코드의 모든 컬럼 선택
SELECT * 
FROM Reservation 
WHERE note IS NULL 
...
# IFNULL (검사 할 인수, NULL일 때의 값)
# 검사 할 인수가 NULL이면 NULL일 때의 값을 반환
...
CREATE TABLE Reservation3 (
 ...
 # CASCADE 옵션 : 참조하는 데이터가 변경되거나 삭제되면 참조 테이블에서도 삭제와 수정이 같이 이루어짐
...
# MySQL 에서는 INNER JOIN 구문을 ,로 대체하고 ON을 WHERE로 대체해서 사용가능
...
# 선택한 컬럼명 앞에 어떤 테이블의 컬럼인지를 직접 지정 해 주는 것이 좋음
...
FROM Reservation AS RV, Room AS RM
...
SELECT * FROM Reservation3
WHERE roomNumber = (SELECT roomNumber FROM Room WHERE roomNumber = 2902);

SELECT * FROM Reservation3
WHERE roomNumber IN (SELECT roomNumber FROM Room);

# FROM 절에서 사용하는 방법

# SELECT 컬럼명
# FROM (SELECT 컬럼명 FROM 테이블 WHERE 조건)
# WHERE 조건;

# VIEW
# 미리 선언된 쿼리를 사용해서 가상의 테이블을 만들어 보여주는 것

# CREATE VIEW 뷰 이름 AS SELECT 쿼리
