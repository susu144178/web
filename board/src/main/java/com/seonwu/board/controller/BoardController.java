...
public class BoardController {
    ...
    public ResponseDto<PostBoardResponseDto> postBoard(@ApiParam(hidden = true) @AuthenticationPrincipal String email, @Valid @RequestBody PostBoardDto requestBody) {
    ...
    @ApiOperation(value = "특정 게시물 수정", notes = "Request Header Athorization에 Bearer JWT를 포함하고 Request Body에 boardNumber, title, content, boardImgUrl을 포함하여 요청을 하면, 성공시 게시물 전체 데이터를 반환, 실패시 실패 메세지를 반환")
    ...
    public ResponseDto<DeleteBoardResponseDto> deleteBoard(@ApiParam(hidden = true) @AuthenticationPrincipal String email, @ApiParam(value = "검색어", example = "1", required = true) @PathVariable("boardNumber") int boardNumber) {
        ResponseDto<DeleteBoardResponseDto> response = boardService.deleteBoard(email, boardNumber);

        return response;
    }

    @ApiOperation(value = "좋아요 기능", notes = "Request Header Ahtorization에 Bearer JWT를 포함하고 Request Body에 boardNumber를 포함하여 요청을 하면, 성공시 게시물 전체 데이터를 반환, 실패시 실패 메세지를 반환")
    @PostMapping(LIKE)
