...
import { SIGN_UP_URL, VALIDATE_EMAIL_URL, VALIDATE_NICKNAME_URL, VALIDATE_TELNUMBER_URL } from "src/constants/api";
...
function FirstPage() {
  ...
  const onPasswordCheckChageHandler = (event: ChangeEvent<HTMLTextAreaElement | HTMLInputElement>) => {
    const value = event.target.value;
    ...
    <Box>
      ...
      <FormControl sx={{ mt: "40px" }} error={signUpError} fullWidth variant="standard" >
        ...
        <Input
          ...
          value={passwordCheck}
          onChange={(event) => onPasswordCheckChageHandler(event)}
        />
        ...
        <Input
          ...
          onChange={(event) => setNickname(event.target.value)}
           ...
           {nicknameValidate === null ? (<></>) :
            nicknameValidate ? 
            (<FormHelperText sx={{ color: 'green' }}>사용 가능한 닉네임입니다.</FormHelperText>) :
      ...
      <FormControl sx={{mt: '40px'}} error={signUpError} fullWidth variant="standard">
        ...
        <Input
          ...
          {telNumberValidate === null ? (<></>) :
            !telNumberPatternCheck ? 
            (<FormHelperText sx={{ color: 'red' }}>전화번호 패턴이 일치하지 않습니다.</FormHelperText>) :
