...
public class Sample01 {
	...
	public static void writeBoard() {		
		...
		// 등록이 완료되면 처음화면으로 돌아옴
		for (int i = 0; 0 < BOARD_LIST.length; i++) {
	...
	public static int getBoardList() {
		...
		for (Board board : BOARD_LIST) {
			// 꺼낸 board 요소에서 null이면 다음 요소를 꺼내옴
	...
	static void setBoard(int i) {
		...
		Scanner wirter = new Scanner(System.in);
		
		System.out.println("타이틀 : ");
		BOARD_LIST[i].title = wirter.nextLine();
		System.out.println("작성자 : ");
		BOARD_LIST[i].writer = wirter.nextLine();
		System.out.println("작성 날짜 및 시간 : ");
		BOARD_LIST[i].writeDate = wirter.nextLine();
		System.out.println("내용 : ");
		BOARD_LIST[i].contents = wirter.nextLine();
