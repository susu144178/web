...
CREATE TABLE User (
 ...
 name VARCHAR(50) NOT NULL,				# 이름 / 길이 50의 가변문자열 / 필수 값
 telNumber VARCHAR(15) NOT NULL UNIQUE	# 전화번호 / 길이 15의 가변문자열 / 필수 값 / 중복 불가능
);

# Board 테이블 생성
# 게시물 번호 (boardNumber)가 존재하고 정수 형태 및 자동 증가로 관리,
# 게시물 번호로 각 데이터를 구분
...
# User 레코드를 삽입
# 아이디는 자동 값을 그대로 사용, 비밀번호는 'P!ssw0rd', 이름 '고길동', 전화번호 '010-4488-9944'인 데이터를 생성
...
VALUES ('안녕히가세요.', '반갑습니다.', '2023-01-11',3);
...
INSERT INTO User (password, name, telNumber) VALUES ('qwer1234', 'Alise', '010-2222-8888');
INSERT INTO User (password, name, telNumber) VALUES ('qlalfqjsgh', 'Brown', '010-3333-6666');
...
INSERT INTO Board (boardTitle, boardContent, boardDateTime, boardWriter)
VALUES ('GoodBye World!', 'GoodBye MySQL', now(), 3);
...
FROM User
WHERE id IN (SELECT DISTINCT boardWriter FROM Board);
